    ...
    private setupLoginSystem(): void {
        ...
        if (player1Input) {
            player1Input.addEventListener('input', (e) => {
                this.player1Name = (e.target as HTMLInputElement).value || 'Blossom';
            });
        }
        
        if (player2Input) {
            player2Input.addEventListener('input', (e) => {
                this.player2Name = (e.target as HTMLInputElement).value || 'Bubbles';
            });
        }
    }
...
// --- Avatar Picker Logic ---
function setupAvatarPicker(profileManager: ProfileManager) {
    const avatarPicker = document.getElementById('avatarPicker');
    const selectedAvatarInput = document.getElementById('selectedAvatar') as HTMLInputElement;
    if (!avatarPicker || !selectedAvatarInput) return;
    ...
    if (profileManager && profileManager.getAvatar) {
        ...
        if (current) {
            ...
            avatarPicker.querySelectorAll('img').forEach(i => {
                if ((i as HTMLImageElement).src.includes(current)) {
...
class ProfileManager {
    ...
    setupRegister() {
        ...
        if (selectedAvatarInput) selectedAvatarInput.value = this.avatar;
        if (registerBtn) {
